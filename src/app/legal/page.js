...
export const metadata = {
  ...
  description:
    "Terms & Conditions, Privacy Policy, Community Guidelines, Child Safety Policy and Cookie Notice for Ments.",
};
...
const TERMS = [
  ...
  {
    num: "1.17",
    title: "Apple & Google Platform Terms",
    body: [
  ...
  },
  {
    num: "1.19",
    title: "Contact",
    body: [`Questions about these Terms? Email us at support@ments.app.`],
  },
];
      ...
      { sub: "Information You Provide Directly" },
      {
        table: {
          ...
          rows: [
            [
              "Account & Identity",
              "Full name, email address, username, password (hashed), profile photo, banner image",
            ],
            [
              "Professional Profile",
              'Tagline, bio, current city, user type, skills, "looking for" interests, social links (LinkedIn, portfolio URLs)',
            ],
            [
              "Experience & Education",
              "Company name, position title, dates, description; institution, degree, field of study, dates",
            ],
            [
              "Resume",
              "Uploaded PDF files, extracted text content (processed by AI for auto-fill)",
            ],
            [
              "Projects & Portfolios",
              "Project title, description, media (images/slides), links, portfolio views",
            ],
            [
              "Startup Profiles",
              "Brand name, mission, funding details, founder information",
            ],
            [
              "Content",
              "Text posts, replies, images, videos, poll questions and responses",
            ],
            [
              "Messages",
              "Direct message content, message reactions, conversation metadata (participants, timestamps, read status)",
            ],
            [
              "Job Applications",
              "Applications submitted, answers to AI interview questions, match scores, profile summaries, application timestamps",
            ],
            [
              "Organization & Events",
              "Organization memberships, event participation, competition entries",
            ],
          ],
        },
      },
      { sub: "Information Collected Automatically" },
      {
        table: {
          ...
          rows: [
            [
              "Device & Technical",
              "Device type, operating system, app version, device platform (iOS/Android)",
            ],
            [
              "Push Notification Tokens",
              "Firebase Cloud Messaging (FCM) token, stored per device",
            ],
            [
              "Usage Data",
              "Content viewed, liked, shared, or interacted with; time spent viewing content; navigation patterns; session identifiers",
            ],
          ],
        },
      },
      ...
      `We never sell your personal data. We share it only with the following:`,
      {
        table: {
          ...
          rows: [
            [
              "Supabase",
              "Backend infrastructure, authentication, database, file storage, real-time messaging",
              "Account data, content, messages, files",
            ],
            [
              "Firebase (Google)",
              "Push notifications",
            ...
            [
              "Google Sign-In",
              "Authentication",
              "OAuth tokens (email, name, profile photo)",
            ],
            [
              "Groq",
              "AI-powered resume parsing, intelligent search, interview question generation, job matching",
              "Resume text, search queries, profile data for matching",
            ],
            [
              "ElevenLabs",
              "Voice agent conversations",
              "Voice audio during active voice sessions only",
            ],
            [
              "AWS S3",
              "File storage (images, videos, resumes)",
    ...
    body: [
      {
        table: {
          ...
          rows: [
            [
              "Camera",
              "Take photos, record videos, scan QR codes at events",
              "When you create a post, update your profile photo, or scan a QR code",
            ],
            [
              "Microphone",
              "Voice agent conversations",
              "When you initiate a voice agent session",
            ],
            [
              "Photo Library",
              "Select existing photos and videos for upload",
              "When you create a post or update your profile photo",
            ],
            [
              "Push Notifications",
              "Receive alerts for messages, replies, mentions, and application updates",
              "On first app launch (you can decline or change later in device settings)",
            ],
          ],
        },
      },
...
const SECTIONS = [
  {
    ...
    meta: "Last updated: March 27, 2026",
    clauses: TERMS,
  },
  {
    id: "privacy",
    ...
    meta: "Last updated: March 27, 2026 — including Data Safety disclosure",
    clauses: PRIVACY,
  },
  {
    id: "community",
  ...
  },
  {
    id: "child-safety",
    n: "04",
    title: "Child Safety Policy (CSAE)",
    clauses: CHILD,
  },
  {
    id: "cookies",
...
function Block({ item, k }) {
  ...
  if (item.sub)
    return (
      <h4 key={k} className="legal-h4">
        {item.sub}
      </h4>
    );
  if (item.note)
    return (
      <p key={k} className="legal-note">
        {item.note}
      </p>
    );
  if (item.caps)
    return (
      <p key={k} className="legal-caps">
        {item.caps}
      </p>
    );
  if (item.table) {
...
function Clause({ clause }) {
  return (
    <div className="legal-clause">
      ...
      {clause.body.map((item, k) => (
        <Block key={k} item={item} k={k} />
      ))}
    </div>
  );
}
...
export default function LegalPage() {
  return (
    <div className="interior document-page legal-page">
        ...
        <div className="container">
          <header className="legal-hero">
            ...
            <p className="legal-intro">
              Welcome to Ments (&ldquo;the App&rdquo;), a professional
              networking and opportunities platform based in India
              (&ldquo;we&rdquo;, &ldquo;us&rdquo;, or &ldquo;our&rdquo;). Below
              you will find the core legal documents required for publishing and
              operating the App on Google Play and the Apple App Store.
            </p>
          </header>
